          ...
          <TabsContent key={cat} value={cat} className="mt-0">
            {filteredProducts.length > 0 ? (
              <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
                ...
                {/* Wrapper Scroll Horizontal (Komentar dipindah kesini agar aman) */}
                          ...
                          >
                            ...
                            <TableCell>
                              <div className="flex flex-col">
                                <span className="font-lora font-medium text-stone-800 text-base group-hover:text-primary transition-colors whitespace-nowrap">
                                  {product.name}
                                </span>
                            ...
                            <TableCell className="text-right font-mono text-stone-700 font-medium whitespace-nowrap">
                              Rp {product.price.toLocaleString('id-ID')}
                            </TableCell>

                            <TableCell className="text-center">
                              {product.stock > 5 ? (
                                 <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border-emerald-100 font-normal whitespace-nowrap">
                                   {product.stock} Tersedia
                                 </Badge>
                              ) : product.stock > 0 ? (
                                 <Badge variant="secondary" className="bg-yellow-50 text-yellow-700 hover:bg-yellow-100 border-yellow-100 font-normal whitespace-nowrap">
                                   Sisa {product.stock}
                                 </Badge>
                              ) : (
                                 <Badge variant="destructive" className="bg-red-50 text-red-600 hover:bg-red-100 border-red-100 font-normal shadow-none whitespace-nowrap">
                                   Habis
                                 </Badge>
                              )}
                            </TableCell>
